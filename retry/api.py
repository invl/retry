...
async def __retry_internal_async(f, exceptions=Exception, tries=-1, delay=0,
                                 max_delay=None, backoff=1, jitter=0,
                                 logger=logging_logger):
    _tries, _delay = tries, delay
    while _tries:
...
def retry(exceptions=Exception, tries=-1, delay=0, max_delay=None, backoff=1,
    ...
    def retry_decorator(f, *fargs, **fkwargs):
        ...
        if not asyncio.iscoroutinefunction(f):
            return __retry_internal(partial(f, *args, **kwargs), exceptions,
                                    tries, delay, max_delay, backoff, jitter,
                                    logger)
        else:
            return __retry_internal_async(partial(f, *args, **kwargs),
                                          exceptions, tries, delay, max_delay,
                                          backoff, jitter, logger)
